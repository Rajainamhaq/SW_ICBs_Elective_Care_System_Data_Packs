...
#create_main_df 
fun_6plus_week_perc_REGION_timeseries <- function(icb_name) {
  ...
  data_rtt_monthly_num <- data_RTT_datamarta |>
    # filter(ICB == "Devon") |>
    filter(Measure == "6+ week %" & (FY %in% get_last_two_fy_labels_monthly())
  ...
  data_rtt_wlmds_num <- data_RTTt |>
    ...
    # filter(ICB == "Devon") |>
    filter(Measure == "6+ week %"  & (FY %in% get_last_two_fy_labels_wlmds())
           & Metric_Type == "Numerator") |>
    mutate(Month_Yr = format(Reporting_Period,"%b-%y")) |>
    group_by(FY, Month,Month_Yr) |>
  ...
  ## get the Target  and Ambition Values for the table below and will get used in box display
    ...
    data_rtt_target <- data_rtt_wlmdst |>
      select(1,2,3) |>
      mutate(FY = "Target (5%)",
             data_type = "Target",
             perf_perc = diag_targetv) |>
      arrange(Reporting_Period) |>
      slice_head(n=12)
    # prepare ambition value for the table
    data_rtt_ambition <- data_rtt_wlmdst |>
      select(1,2,3) |>
      mutate(FY = "Ambition (15%)",
             data_type = "Ambition",
             perf_perc = ambitionv) |>
      arrange(Reporting_Period) |>
      slice_head(n=12)
    ...
    data_rtt_ambition$Reporting_Period <- as.Date(data_rtt_ambition$Reporting_Period)
  ...
  names(data_rtt_target) <- names(data_rtt_monthlyt)
    names(data_rtt_ambition) <- names(data_rtt_monthlyt)
#  names(data_rtt_perf_plan) <- names(data_rtt_monthlyt)  
  df_for_series <- rbind(data_rtt_monthlyt,data_rtt_wlmdst,data_rtt_target,data_rtt_ambition) 
  ...
  df_for_series <- df_for_series |>
    ...
    mutate(
      date_timeseries = case_when(
        FY == get_last_two_fy_labels_wlmds()[2] ~ Reporting_Period %m-% years(1),
