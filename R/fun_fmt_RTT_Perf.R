...
####################################################################################
fun_fmt_RTT_Perf <- function(RTT_tablet) {
  ...
  RTT_tablet |>
    ...
    tab_header(title = "RTT Performance by Specialty") |>
    ...
    cols_label(RTT_percentage = "RTT Performance") |>
...
#  )
  ...
  tab_footnote(
    footnote = md(paste0("Note: Table displays RTT Performance by Specialty with the % point change highlighted pink if below Baseline and green if above target.
                          \nData sources: Baseline is the monthly published Nov-24 statistics and WLMDS for w/e ", week_ending_fn))
  ) |>
    ...
    cols_label(
      Baseline = get_current_fy_baseline(),
      Target = get_mar_target_label()
