...
### get max dates for foot note ####
    max_date <- df_for_series |>
      filter(FY == "25/26 monthly" & data_tye == "RTT Monthly")
...
## plot the time series    
 #df_timeseries |>
   df_for_series |>
    ...
    scale_y_continuous(limits = c(0, 16),labels = label_number(accuracy = 1)) + 
    labs(x = NULL, y = NULL, title = "Spec Advice utilisation - pre referral") +
    # theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
    theme_bw() +
    labs(
      caption = paste0("Data sources: System EROC to ", week_ending_fn)
    )+
    theme(
