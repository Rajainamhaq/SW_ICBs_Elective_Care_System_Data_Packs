...
####################################################################################
fun_fmt_31days_comb_std <- function(df_31days_comb_std_table) {
  
  max_date_sus_opa <- format(max_date_sus_opa,"%b-%y")
  
  df_31days_comb_std_table
  df_31days_comb_std_table |>
    gt()  |>                 # put into a gt table
    tab_header(title = "31-day Combined Standard (25/26 target = 96%)") |> 
  ...
  tab_style(
    ...
    locations = cells_body(
      columns = 1,
      rows = nrow(df_31days_comb_std_table))
  ) |>
  ...
  tab_style(
    ...
    locations = cells_body(rows=nrow(df_31days_comb_std_table)-1)
  ) |>
  ...
  tab_style(
    ...
    tab_footnote(
      footnote = md(paste0("Numerator = Number of people receiving treatment for cancer within 31 days
