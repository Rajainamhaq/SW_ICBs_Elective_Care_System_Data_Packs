...
####################################################################################
fun_fmt_pre_referral_utilisation <- function(RTT_tablet) {
  
 # week_ending_eroc <- max(RTT_tablet$R)
  
  RTT_tablet
  RTT_tablet |>
    gt()  |>                 # put into a gt table
    tab_header(title = "Specialist Advice utilisation - pre referral by specialty") |> 
    ...
    ##cols_label(wait_to_first_percentage = "Wait to first") |>
  ...
  tab_style(
    ...
    tab_footnote(
      footnote = md(paste0("Note: Table displays Specialist Advice utilisation by specialty and is green where utilisation exceeds the target.
                           \nData sources: System EROC and SUS OPA for ", format(max_date_sys_eroc, "%b-%y")))
    ) |>
