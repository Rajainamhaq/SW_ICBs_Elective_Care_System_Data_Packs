####################################################################################
####### table formatting for Diagnostic Activity table ####
####################################################################################
fun_fmt_diag_activity <- function(RTT_tablet) {
  ...
  RTT_tablet |>
    ...
    tab_header(title = "Diagnostic activity by modality") |>
 ...
 #   fmt_percent(columns = 5, decimals = 1, scale_values = FALSE) |>
    ...
    cols_align(align = "left", columns = c("Modality")) |>
    ## color the 1st Col
    data_color(
      columns = c("Modality"),
      colors = "#005eb8"
  ...
  tab_style(
    ...
    tab_footnote(
      footnote = md(paste0("Data source: DM01 for ",  format(max_date_DM01,"%b-%y")))
    ) |>
