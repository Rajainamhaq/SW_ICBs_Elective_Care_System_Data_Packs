...
####################################################################################
fun_fmt_28days_fds <- function(df_28days_fds_table) {
  ...
  tab_style(
    ...
    locations = cells_body(rows=nrow(df_28days_fds_table)-1)
  ) |>
  ...
  tab_style(
    ...
    tab_footnote(
      footnote = md(paste0("Numerator = Number of people told cancer diagnosis outcome within 28 days
                           \nDenominator = Total number of people told cancer diagnosis outcome
                           \nData source: Cancer Waiting Times for ",
                           format(max_date_sus_opa)))
        ) |>
