...
ip_spec_acute_elec_ord_activity_ICB_timeseries <- function(icb_name) {
    ...
    data_ip_acute_ord_daycase <- data_RTT_datamarta |>
      # filter(ICB == "Devon") |>
      filter(Measure == "KB: Elec. Ord. (Unaj.)" & (FY %in% get_last_two_fy_labels_monthly())
             & Metric_Type == "Count") |>
    ...
    data_ip_activtiy_plan <- data_RTT_datamarta |>
      filter(Measure == "KB: Elec. Ord. (Unaj.)") |>
    ...
    ip_spec_acute_final <- rbind(data_ip_acute_ord_daycase, data_ip_activtiy_plan) 
