...
fun_pifu_utilisation_ICB <- function(icb_name) {

Target_pifu_utilisatoin <- 5
...
pifu_utilisation <- rbind(RTT_tablet_top10, RTT_table_allother_total, RTT_table_total_pathways )
pifu_utilisation <- pifu_utilisation |>
  select(Specialty, Numerator, Denominator, Utilisation) 


######## End of RTT Performance Table creation ###############################

return(pifu_utilisation)
