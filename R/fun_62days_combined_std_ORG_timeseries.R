...
#create_main_df 
fun_62days_combined_std_ORG_timeseries <- function(org_code) {
  ...
  data_rtt_monthly_num <- data_RTT_datamarta |>
    # filter(ICB == "Devon") |>
    filter(Measure == "SW CWT 62d Combined by Tumour Site" & (FY %in% get_last_two_fy_labels_monthly())
  ...
  data_rtt_perf_plan <- data_RTT_datamarta |>
    ...
    # filter(ICB == "Devon") |>
    filter(Measure == "SW CWT 62d Combined by Tumour Site") |>
...
#  names(data_rtt_perf_plan) <- names(data_rtt_monthlyt)  
  df_cwt_62days_comb <- rbind(data_rtt_monthlyt,data_rtt_perf_plan,data_rtt_target) 
  
    df_cwt_62days_comb <- df_cwt_62days_comb |>
    mutate(
      date_timeseries = ifelse( FY == get_last_two_fy_labels_monthly()[2],Reporting_Period %m-% years(1), Reporting_Period ))
       
    df_cwt_62days_comb$date_timeseries <- as.Date(df_cwt_62days_comb$date_timeseries)
  
   return(df_cwt_62days_comb)
