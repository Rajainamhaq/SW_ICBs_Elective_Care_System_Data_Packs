...
df_plans_pub <- data_RTT_datamart |>
...
if (nrow(df_plans_pub) == 0) {
  data_RTT_datamart <- data_RTT_datamart |>
    bind_rows(
      data_RTT_datamart |>
...
# to remove the duplicated values created for plans 

  


#df_RTT_datamart_plan_check <- data_RTT_datamart |>
#  mutate(
#    Reporting_Period = if (nrow(df_plans_pub) > 0) {
#      Reporting_Period
#    } else {
#      if_else(
#        Data_Source == "Plans",
#        Reporting_Period %m+% years(1),
 #       Reporting_Period
#      )
#    }
#  )
...
###################################################################################################################
## Temporary assign the 25/26 plans as 26/27 plans until actual 26/27 plans are released and stored in data warehouse.
###################################################################################################################
#library(dplyr)
#library(lubridate)
#data_RTT_datamarta <- data_RTT_datamarta |>
  #mutate(FY = ifelse(FY=="25/26 plans", "26/27 plans",FY)) |>
 # mutate(Reporting_Period = ifelse(FY=="26/27 plans",Reporting_Period+years(1),Reporting_Period))
