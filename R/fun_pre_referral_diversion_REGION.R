...
#create_main_df 
fun_pre_referral_diversion_REGION <- function(OrgCode) {
  ...
  Spec_adv_diversion <- rbind(RTT_tablet_top10, RTT_table_allother_total, RTT_table_total_pathways )
  Spec_adv_diversion <- Spec_adv_diversion |>
    select(1,3,2,4,5)
  
  
  ######## End of RTT Performance Table creation ###############################
  
  return(Spec_adv_diversion)
}