...
#create_main_df 
fun_28days_faster_diag_std_ICB_timeseries <- function(icb_name) {
  ...
  data_rtt_monthly_num <- data_RTT_datamarta |>
    # filter(ICB == "Devon") |>
    filter(Measure == "SW CWT 28d FDS" & (FY %in% get_last_two_fy_labels_monthly())
...
#  names(data_rtt_perf_plan) <- names(data_rtt_monthlyt)  
  df_cwt_28_fds <- rbind(data_rtt_monthlyt,data_rtt_perf_plan,data_rtt_target) 
  
    df_cwt_28_fds <- df_cwt_28_fds |>
    mutate(
      date_timeseries = ifelse( FY == get_last_two_fy_labels_monthly()[2],Reporting_Period %m-% years(1), Reporting_Period ))
       
    df_cwt_28_fds$date_timeseries <- as.Date(df_cwt_28_fds$date_timeseries)
  
   return(df_cwt_28_fds)
