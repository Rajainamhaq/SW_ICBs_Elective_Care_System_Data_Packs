...
fun_pre_referral_diversion_Plot_timeseries <- function(df_for_series) {
...
## plot the time series    
 #df_timeseries |>
   df_for_series |>
    ...
    scale_y_continuous(limits = c(0.1, 0.8),labels = label_percent(accuracy = 1)) + 
    labs(x = NULL, y = NULL, title = NULL) +
    # theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
    theme_bw() +
    labs(
      caption = paste0("Data source: System EROC to ", week_ending_fn)
    )+
    theme(
