...
new_fy_date <- as.Date("2026-04-01") # updated date to 2026 from 2025
#new_fy_date <- get_last_two_fyset_start() # this makes the date dynamic 
...
data_RTT_datamarta <- data_RTT_datamart |>
  ...
  mutate(Month = month_lookup$Mon_name[match(month(Reporting_Period),month_lookup$Mon_num)]) |>
  mutate(FY = ifelse(Data_Source == "Plans","26/27 plans",
                     ifelse(Reporting_Period < new_fy_date,"26/27 monthly","27/28 monthly")
  )) 
...
# and look up table to be used is spec_mod_lookup_RTT
...
################# End of main data frame creation ################################################
  ...
  data_SUS_OPAt <- data_SUS_OPAt |>
    ...
    mutate(Reporting_Period = as.Date(Der_Activity_Month)) |>
