...
ip_spec_acute_elec_ord_activity_Plot_timeseries <- function(ip_spec_acute_final) {
###### get max dates for foot note ####
    ...
    min_y <- floor(min(ip_spec_acute_final$ip_acute_daycase_activty)/100)*100
    max_y <- ceiling(max(ip_spec_acute_final$ip_acute_daycase_activty)/100)*100
...
## plot the time series    
  ...
  #  scale_linetype_manual(values = c("dashed", "solid", "solid","dotted")) +  # Specify dotted lines
   # scale_color_manual(values = c("#005EB8",  "#005EB8", "#AE2573","red")) + 
    ...
    theme_bw() +
    labs(
      caption = paste0("Data source: SUS APCS specific acute elective ordinary activity to ",max_rtt_date_fn)
    )+
    theme(
