...
key_summary_main <- bind_cols(rtt_performance, wait_to_first, fiftytwoplusweeks, 
 ...
 make_label <- function(title, date, fmt = "%b-%y") {
   gt::md(paste0(title, "  \n", format(date, fmt)))
 }
 ...
 labels <- c(
   ...
   RTT_monthly = make_label("RTT monthly", rtt_monthly_max_date),
   Plans_RTT_mthly = make_label("Plans", rtt_monthly_max_date),
   
   WLMDS_RTT_weekly = make_label("WLMDS", rtt_wlmds_max_date, "%d-%b-%y"),
   Plan_RTT_weekly = make_label("Plans", rtt_wlmds_max_date),
   
   WLMDS_wait_to_1st = make_label("WLMDS", wait_to_1st_wlmds_max_date, "%d-%b-%y"),
   Plan_wait_to_1st = make_label("Plans", wait_to_1st_wlmds_max_date),
   
   RTT_monthly_52plus = make_label("RTT monthly", monthly_52plus_max_date),
   Plans_52plus_mthly = make_label("Plans", monthly_52plus_max_date),
   
   WLMDS_52plus = make_label("WLMDS", wlmds_52plus_max_date, "%d-%b-%y"),
   Plans_52plus_weekly = make_label("Plans", wlmds_52plus_max_date),
   
   DM01 = make_label("DM01", dm01_diag_max_date),
   Plans_diag = make_label("Plans", dm01_diag_max_date),
   WL_or_Planned_perc_diag = make_label("% WL or Planned", dm01_diag_max_date),
   
   Utilisation = make_label("Utilisation", ag_utilisation_max_date),
   Diversion = make_label("Diversion", ag_diversion_max_date),
   
   WLMDS_12plus_week_val = make_label("WLMDS", twelve_plus_week_val_max_date, "%d-%b-%y"),
   
   SUS_OPA = make_label("SUS-OPA", missed_appt_max_date),
   
   EROC_SUS_OPA = make_label("EROC_SUS_OPA", pifu_eroc_sus_opa_denom_max_date),
   Plan_pifu_utilisation = make_label("Plans", pifu_eroc_sus_opa_denom_max_date)
 )
