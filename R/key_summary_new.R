...
key_summary_main <- bind_cols(rtt_performance, wait_to_first, fiftytwoplusweeks, 
 ...
 names(key_summary_main) <- gsub("Plans_diag","Plans",names(key_summary_main))
 ...
 names(key_summary_main) <- gsub("EROC_SUS_OPA","EROC/SUS OPA",names(key_summary_main))
 ...
 labels <- names(key_summary_main) |>
  ...
  names(key_summary_main) <- labels
 
 ## make names unique
 key_summary_main1 <- key_summary_main 
 names(key_summary_main1) <- make.unique(names(key_summary_main1))
 
 # store original names to be used later
 original_labels <- gsub("\\..*$", "", names(key_summary_main1))
 label_map <- setNames(original_labels, names(key_summary_main1))
 
 
## Now create a gt table to render in html file with all the formatting
