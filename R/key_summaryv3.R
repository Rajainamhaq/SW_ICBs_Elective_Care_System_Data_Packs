...
row_dates <- data.frame(ICB = "", 
 ...
 ## before combining 3 data frames into one have to check data types bec for row_dates its all chr not numeric
 #library(data.table)
 
 ## combine the means row with the main table

 
  key_summary_main1 <- bind_rows(key_summary_main, South_West)
# key_summary_main1 <- rbindlist(list(row_dates, key_summary_main, South_West), fill = TRUE)
 
 ## as rbindlist converts all the values in data frame to chr so now after binding we can switch back except 1st two cols
 library(data.table)
 
 # convert only rows 2+ and columns 3+
# key_summary_main1[2:.N, (3:ncol(key_summary_main1)) :=
 #                   lapply(.SD, as.numeric),
  #                .SDcols = 3:ncol(key_summary_main1)]
 ...
 names(key_summary_main1) <- gsub("RTT_monthly","RTT monthly",names(key_summary_main1))
 names(key_summary_main1) <- gsub("Plans_RTT_mthly","Plans",names(key_summary_main1))
 names(key_summary_main1) <- gsub("WLMDS_RTT_weekly","WLMDS1",names(key_summary_main1))
 names(key_summary_main1) <- gsub("Plan_RTT_weekly","Plans1",names(key_summary_main1))
 names(key_summary_main1) <- gsub("WLMDS_wait_to_1st","WLMDS2 ",names(key_summary_main1))
 names(key_summary_main1) <- gsub("Plan_wait_to_1st","Plans2",names(key_summary_main1))
 names(key_summary_main1) <- gsub("RTT monthly_52plus","RTT monthly1",names(key_summary_main1))
 names(key_summary_main1) <- gsub("Plans_52plus_mthly","Plans3",names(key_summary_main1))
 names(key_summary_main1) <- gsub("WLMDS_52plus","WLMDS3",names(key_summary_main1))
 names(key_summary_main1) <- gsub("Plans_52plus_weekly","Plans4",names(key_summary_main1))
 names(key_summary_main1) <- gsub("Plans_diag","Plan",names(key_summary_main1))
 names(key_summary_main1) <- gsub("WL_or_Planned_perc_diag","% WL or Planned",names(key_summary_main1))
 names(key_summary_main1) <- gsub("WLMDS_12plus_week_val","WLMDS4",names(key_summary_main1))
 names(key_summary_main1) <- gsub("SUS_OPA","SUS OPA",names(key_summary_main1))
 names(key_summary_main1) <- gsub("EROC_SUS_OPA","EROC/SUS OPA",names(key_summary_main1))
 names(key_summary_main1) <- gsub("Plan_pifu_utilisation","Plans5",names(key_summary_main1))

 ## set up new labels that take off numbers from Plan and WLMDs to put in gt
 labels <- names(key_summary_main1) |>
   str_replace_all("^(Plans|WLMDS|RTT monthly)\\d+\\s*", "\\1 ") |>
   str_squish() |>
   setNames(names(key_summary_main1))  

 
 ## Now create a gt table to render in html file with all the formatings
 
 key_summary_gt <- key_summary_main1 |>  
 ...
 #  fmt_number(columns = 13, decimals = 1, scale_by = TRUE) |>
  # fmt_number(columns = 14, decimals = 1, scale_by = TRUE) |>
   fmt_percent(columns = 15, decimals = 1, scale_values = TRUE) |>
  fmt_number(columns = 16, decimals = 1, scale_by = 1) |>
  ...
  tab_style(
    ...
    locations = cells_body(rows = nrow(key_summary_main1))
  ) |>
   ...
   tab_style(
     ...
     locations = cells_body(rows=nrow(key_summary_main1)-1)
   ) |>
   tab_style(
     style = list(
       cell_borders(
  ...
  # cols_label(RTT_monthly = "RTT monthly") |>
   ...
   tab_style(
     ...
     locations = cells_body(
       columns = 3,
       rows = key_summary_main1[[3]] > key_summary_main1[[4]]
     )
   )  |>
   ...
   tab_style(
     ...
     locations = cells_body(
       columns = 5,
       rows = key_summary_main1[[5]] > key_summary_main1[[6]]
     )
   )  |>
   ...
   tab_style(
     ...
     locations = cells_body(
       columns = 7,
       rows = key_summary_main1[[7]] > key_summary_main1[[8]]
     )
   )  |>
   ...
   tab_style(
     ...
     locations = cells_body(
       columns = 9,
       rows = key_summary_main1[[9]] <= key_summary_main1[[10]]
     )
   )  |>
   tab_style(
     style = cell_fill(color = "#E2EFDA"),
     locations = cells_body(
       columns = 11,
       rows = key_summary_main1[[11]] <= key_summary_main1[[12]]
     )
   )  |>
   ...
   tab_style(
     ...
     locations = cells_body(
       columns = 13,
       rows = key_summary_main1[[13]] <= key_summary_main1[[14]]
     )
   )  |>
   ...
   tab_style(
     ...
     locations = cells_body(
       columns = 20,
       rows = key_summary_main1[[20]] > key_summary_main1[[21]]
     )
   )  |>
  ...
  ## add the main headings for each secion in the table. 
   tab_spanner(
     ...
     columns = c(colnames(key_summary_main1)[3], colnames(key_summary_main1)[4],
                    colnames(key_summary_main1)[5],colnames(key_summary_main1)[6])) |>
   tab_spanner(
     label = "Wait to first",
     columns = c(colnames(key_summary_main1)[7], colnames(key_summary_main1)[8])) |>
   tab_spanner(
     label = "52+ week %",
     columns = c(colnames(key_summary_main1)[9], colnames(key_summary_main1)[10],
                    colnames(key_summary_main1)[11], colnames(key_summary_main1)[12]))|>
   tab_spanner(
     label = "Diagnostic activity",
     columns = c(colnames(key_summary_main1)[13], colnames(key_summary_main1)[14],
                    colnames(key_summary_main1)[15])) |>
    tab_spanner(
     label = "A&G: Pre referral",
     columns = c(colnames(key_summary_main1)[16], colnames(key_summary_main1)[17]))|>
   tab_spanner(
     label = "12+ week validation",
     columns = c(colnames(key_summary_main1)[18]))|>
   tab_spanner(
     label = "Missed appt",
     columns = c(colnames(key_summary_main1)[19]))|>
   tab_spanner(
     label = "PIFU utilisation",
     columns = c(colnames(key_summary_main1)[20], colnames(key_summary_main1)[21]))|>
 #  tab_spanner(
