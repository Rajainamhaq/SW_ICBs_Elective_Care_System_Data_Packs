...
###  Spec Advice diversion - pre referral line chart ######
####################################################################

#create_main_df 
fun_pre_referral_diversion_REGION_timeseries <- function(icb_name) {
 ...
 # data_RTTt <- data_RTTt 
  ...
  data_rtt_monthly_num <- data_RTT_datamarta |>
    # filter(ICB == "Devon") |>
    filter(Measure == "Spec Advice diversion - pre referral" & (FY %in% get_last_two_fy_labels_monthly())
  ...
  data_target <- data.frame(
    ...
    FY = "Limit (45%)",
    data_tye = "Limit",
    perf_perc = Targetv_spec_adv_div
  )
