...
#create_main_df 
fun_Wait_to_1st_Activity_ICB_timeseries <- function(icb_name) {
  ...
  ## dont needt 1st table for Wait to 1st activity timeseires plot
  ...
  data_rtt_perf_plan <- data_RTT_datamarta |>
    # filter(`Org Short Name` == "R Devon Uni") |>
  #   filter(ICB == "Devon") |>
    filter(Measure == "Wait to first"  & FY == get_current_fy_plans_label()) |>
  ...
  df_for_series <- rbind(data_rtt_wlmdst,data_rtt_perf_plan) #,data_rtt_target)  to take out target and basleine form region and Icb
