...
####################################################################################
fun_fmt_6plus_week_perc <- function(RTT_tablet) {
  
  
     # RTT_tablet
  RTT_tablet |>
    gt()  |>                 # put into a gt table
    tab_header(title = "Diagnostic Performance by modality") |>
   ...
   # fmt_percent(columns = 6, decimals = 1, scale_values = FALSE) |>
    ...
    cols_align(align = "left", columns = vars("Modality")) |>
    ## color the 1st Col
    data_color(
      columns = vars("Modality"),
      colors = "#005eb8"
 ...
 #   tab_style(
  #    style = cell_fill(color = "#ffc7ce"),
   ...
   #   ) ) |>
    ...
    tab_footnote(
      footnote = md(paste0("Numerator = Number of patients waiting 6+ weeks
