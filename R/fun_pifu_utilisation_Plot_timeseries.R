...
fun_pifu_utilisation_Plot_timeseries <- function(df_pifu_utilisation) {
###### get max dates for foot note ####
    ...
    min_y_axis <- min(df_pifu_utilisation$perf_perc,na.rm = TRUE)-0.002
    max_y_axis <- max(df_pifu_utilisation$perf_perc,na.rm = TRUE)+0.002
    # Compute dynamic "nice" breaks (0.5% = 0.005)
    lower_break <- floor(min_y_axis / 0.002) * 0.002
 ...
 #     color = "red", size = 3  # Text styling for "Group 2"
  #  ) +
      scale_y_continuous(limits = c(lower_break, upper_break),
                         breaks = seq(lower_break,upper_break, by = 0.005 ), labels = label_percent(accuracy = 0.1)) + 
    labs(x = NULL, y = NULL, title = NULL) +
    # theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
    theme_bw() +
    labs(
      caption = paste0("Data sources: Provider EROC and SUS OPA to ",max_rtt_date_fn)
    )+
    theme(
