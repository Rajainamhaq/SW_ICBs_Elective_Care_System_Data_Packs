...
####################################################################################
fun_fmt_missed_appointments <- function(RTT_tablet) {
  
  RTT_tablet
  RTT_tablet |>
    gt()  |>                 # put into a gt table
    tab_header(title = "Missed appointments") |> 
 ...
 #     columns = vars(Limit),
  #    rows = Limit > 0
   # ) ) |>
    ...
    tab_footnote(
      footnote = md(paste0("Data source: SUS OPA for ",format(max_date_sus_opa, "%b-%y")))
    ) |>
