...
ip_spec_acute_day_case_activity_Plot_timeseries_ICB <- function(ip_spec_acute_final) {
###### get max dates for foot note ####
    ...
    min_y <- floor(min(ip_spec_acute_final$ip_acute_daycase_activty)/1000)*1000 
    max_y <- ceiling(max(ip_spec_acute_final$ip_acute_daycase_activty)/1000)*1000 
...
## plot the time series    
    ...
    geom_line(size=1) +
     ...
     # geom_text(
    #    data = filter(ip_spec_acute_final, FY == "25/26 monthly"),
    #    aes(label = round(ip_acute_daycase_activty, 0)),
    #    vjust = -0.5,
    #    size = 3,
    #    show.legend = FALSE
    #  )+
  ...
  #  scale_linetype_manual(values = c("dashed", "solid", "solid","dotted")) +  # Specify dotted lines
   # scale_color_manual(values = c("#005EB8",  "#005EB8", "#AE2573","red")) + 
      ...
      fy_linetype_scale_all() +
       ...
       scale_y_continuous(
      breaks = seq(min_y,max_y, by = 1000 ),
     # labels = scales::comma) + 
     labels = label_number()) +
    ...
    theme_bw() +
    labs(
      caption = paste0("Data source: SUS APCS specific acute day case activity to ",max_rtt_date_fn)
    )+
    theme(
