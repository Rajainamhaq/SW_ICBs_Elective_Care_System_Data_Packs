...
####################################################################################
fun_fmt_pifu_utilisation <- function(RTT_tablet) {
  
  RTT_tablet
  RTT_tablet |>
   # pifu_utilisation |>
    gt()  |>                 # put into a gt table
    tab_header(title = "PIFU Utilisation") |> 
 ...
 #     columns = c(Limit),
  #    rows = Limit > 0
   # ) ) |>
    ...
    tab_footnote(
      footnote = md(paste0("Data sources: Provider EROC and SUS OPA for ",format(max_date_sus_opa,"%b-%y")))
    ) |>
