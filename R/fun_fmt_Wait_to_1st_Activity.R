####################################################################################
####### table formatting for Wait for 1st Activity slide ####
####################################################################################
fun_fmt_Wait_to_1st_Activity <- function(RTT_tablet) {
  
  if (nrow(RTT_tablet) > 1) {
  RTT_tablet |>
    gt()  |>                 # put into a gt table
    tab_header(title = "Wait to first activity") |> 
...
#  )
    ...
    tab_footnote(
      footnote = md(paste0("Note: Table displays wait to first activity by Specialty with the % point change and highlights pink if below Baseline and green if above target.
