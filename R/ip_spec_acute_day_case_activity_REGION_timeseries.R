...
ip_spec_acute_day_case_activity_REGION_timeseries <- function(icb_name) {
    ...
    data_ip_acute_daycase <- data_RTT_datamarta |>
      # filter(ICB == "Devon") |>
      filter(Measure == "JB: Day Case (Unaj.)" & (FY %in% get_last_two_fy_labels_monthly())
             & Metric_Type == "Count") |>
    ...
    data_ip_activtiy_plan <- data_RTT_datamarta |>
      filter(Measure == "JB: Day Case (Unaj.)") |>
