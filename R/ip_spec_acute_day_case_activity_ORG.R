...
ip_spec_acute_day_case_activity_ORG <- function(OrgCode) {
    ...
    data_ip_acute_daycase <- data_RTT_datamarta |>
      # filter(ICB == "Devon") |>
      filter(Measure == "JB: Day Case (Unaj.)" & (FY == "24/25 monthly" | FY == "25/26 monthly")
             & Metric_Type == "Count") |>
    ...
    df_pifu_utilisation <- df_pifu_utilisation |>
      mutate(
        date_timeseries = ifelse( FY == "25/26 monthly",Reporting_Period %m-% years(1), Reporting_Period ))
