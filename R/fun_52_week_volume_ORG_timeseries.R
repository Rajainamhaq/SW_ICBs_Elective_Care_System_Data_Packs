...
#create_main_df 
fun_52_week_volume_ORG_timeseries <- function(org_code) {
  ...
  data_rtt_monthly <- data_RTT_datamarta |>
    # filter(ICB == "Devon") |>
    filter(Measure == "52+ week %" & (FY == "24/25 monthly" | FY == "25/26 monthly")
  ...
  data_rtt_wlmds <- data_RTTt |>
    ...
    # filter(ICB == "Devon") |>
    filter(Measure == "52+ week %"  & (FY == "24/25 WLMDS" | FY == "25/26 WLMDS")
           & Metric_Type == "Numerator") |>
    mutate(Month_Yr = format(Reporting_Period,"%b-%y")) |>
    group_by(FY, Month,Month_Yr) |>
    summarise(Metric_Value = sum(Metric_Value),.groups = "drop") |>
  ...
  df_for_series_52_week_volumes <- df_for_series |>
  ...
  df_for_series_52_week_volumes$date_timeseries <- as.Date(df_for_series_52_week_volumes$date_timeseries)
  
   return(df_for_series_52_week_volumes)
