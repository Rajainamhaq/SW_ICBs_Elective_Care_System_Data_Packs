...
####################################################################################
fun_fmt_pre_referral_diversion <- function(RTT_tablet) {
  
  RTT_tablet
  RTT_tablet |>
    gt()  |>                 # put into a gt table
    tab_header(title = "Specialist Advice diversion - pre referral by specialty") |> 
  ...
  tab_style(
    ...
    tab_footnote(
      footnote = md(paste0("Note: Table displays Specialist Advice diversion by specialty and is orange where diversion exceeds the suggested limit.
                           \nData source: System EROC for ",format(max_date_sys_eroc, "%b-%y")))
    ) |>
