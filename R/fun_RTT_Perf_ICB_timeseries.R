...
#create_main_df 
fun_RTT_Perf_ICB_timeseries <- function(icb_name) {
  ...
  data_rtt_monthly_num <- data_RTT_datamarta |>
    # filter(ICB == "Devon") |>
    filter(Measure == "RTT Performance" & (FY %in% get_last_two_fy_labels_monthly())
  ...
  data_rtt_wlmds_num <- data_RTTt |>
    ...
    # filter(ICB == "Devon") |>
    filter(Measure == "RTT Performance" & (FY %in% get_last_two_fy_labels_wlmds())
           & Metric_Type == "Numerator") |>
    mutate(Month_Yr = format(Reporting_Period,"%b-%y")) |>
    group_by(FY, Month,Month_Yr) |>
  ...
  ## has different code for ICB and Region than Org fun
  data_rtt_perf_plan <- data_RTT_datamarta |>
    # filter(`Org Short Name` == "R Devon Uni") |>
  #   filter(ICB == "Devon") |>
    filter(Measure == "RTT Performance"  & FY == get_current_fy_plans_label()) |>
   # select(10,2,17,1,16,7,8,19,20) |>
    select(ICB,`Org Short Name`,Organisation_Code, Reporting_Period, Metric_Type, Metric_Value, Month, FY ) |>
  ...
  rtt_target <- data_RTT_datamarta |>
    filter(Data_Source == "Targets and baselines" & Measure == get_mar_target_label()
           & Measure_Type == "RTT Performance") |>
    #  filter(Organisation_Code =="RH8") |>
    select(Metric_Value)
  rtt_targetv <- mean(unlist(rtt_target))
  
  rtt_baseline <- data_RTT_datamarta |>
    filter(Data_Source == "Targets and baselines" & Measure == "Nov-24 baseline"
           & Measure_Type == "RTT Performance") |>
    #    filter(Organisation_Code =="RH8") |>
    select(Metric_Value)
  rtt_baselinev <- mean(unlist(rtt_baseline))
  ...
  df_for_series <- rbind(data_rtt_monthlyt,data_rtt_wlmdst,data_rtt_perf_plan) #,data_rtt_target) to take out target and baseline lines
